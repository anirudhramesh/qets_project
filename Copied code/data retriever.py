...
def read_file(file_path, chunksize):
    ...
    for chunk in pd.read_csv(file_path, chunksize=chunksize, parse_dates=[0], infer_datetime_format=True):
        chunk = chunk.resample('5min', on='DATE_TIME_M')[['BID', 'BIDSIZ', 'ASK', 'ASKSIZ']].ohlc()
    ...
    frame.fillna(method='ffill', inplace=True, axis=1)
    frame = frame.reset_index().drop_duplicates('DATE_TIME_M', keep='first').set_index('DATE_TIME_M', drop=True)
